...
You can also use Rack::Test without an explicit app method.  In this case it
will check for a CONFIG_RU constant and if that is missing look for a 
config.ru one level up from the test file
  ...
  class HomepageTest < Test::Unit::TestCase
    ...
    #Only needed if it varies from this
